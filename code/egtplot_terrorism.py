...
def grid_ics():
    ...
    print(n,min_x, min_y, max_x, max_y )
    
    
    t=np.array([[min_x,min_y],[max_x,min_y],[(max_x-min_x)/2.+min_x,max_y]])   
    points=triangle_grid_points ( 15, t )
...
#    n_cols, n_rows = get_cols_and_rows(payoff_entries)
    n_cols=subplot[2]; n_rows =subplot[1];
    #n_cols=1; n_rows =1;
...
#            print('==',yout)
            yout=np.array([[ics[x][0],ics[x][1],ics[x][2]], aux.PopulationDynamics(ics[x],payoff_mat)])
            plot_values = np.dot(proj, yout.T)
...
########### if we want arrows with different colours ###############            
#            
#        distmax=max(tri_contour[:,2])
#        distmin=min(tri_contour[:,2]) 
#
#        for x in range(len(ics)):
#            
#            yout=np.array([[ics[x][0],ics[x][1],ics[x][2]], aux.PopulationDynamics(ics[x],payoff_mat)])
...
##                tri_contour[x] = [xx[0], yy[0], dist[0]]
#            
#                ic_color= cmap((dist[0]-distmin)/(distmax-distmin))  # added
# ##############################################################
            try:             
                xx[1]=xx[0]+(xx[1]-xx[0])/(dist[0]*1000)
                yy[1]=yy[0]+(yy[1]-yy[0])/(dist[0]*1000)
...
#            equilibria(payoff_mat, ax)
        ...
        if disppoints==True:              
            for x in range(len(points)):
                plot_values = np.dot(proj, points[x,0:3].T)
                ax.scatter(
                    [plot_values[0]],
                    [plot_values[1]],
                    s=200,
                    #color="red",
                    facecolor=3*(1-points[x,3],),
                    marker="o",
                    zorder=10,
                    )
                #print(points[x])
                #print(plot_values[0],plot_values[1])
...
#        # plot the payoff matrix parameters if appropriate
        if display_parameters:
            Z,N,H1,cW,cC,rW,rC,punP0,punP,punC,gam,waid,TN,tranWC=payoff_entries
            fs=24
            dow=0.15
            plt.text(-1, 1, r'$\beta_S$='+str("{:.0f}".format(round(punP0,0))), fontsize=fs, verticalalignment='top')
            plt.text(-1, 1-dow, r'$\beta_H$='+str("{:.0f}".format(round(punP,0))), fontsize=fs, verticalalignment='top')
            plt.text(-1, 1-2*dow, r'$\beta_C$='+str("{:.0f}".format(round(punC,0))), fontsize=fs, verticalalignment='top')
            if TN==1 or TN==0:
                plt.text(-1, 1-3.5*dow, r'$\delta$='+str("{:.0f}".format(round(TN,0))), fontsize=fs, verticalalignment='top')
            else:   
                plt.text(-1, 1-3.5*dow, r'$\delta$='+str("{:.1f}".format(round(TN,1))), fontsize=fs, verticalalignment='top')
            if tranWC==1 or tranWC==0:
                plt.text(-1, 1-4.5*dow, r'$\tau$='+str("{:.0f}".format(round(tranWC,0))), fontsize=fs, verticalalignment='top')
            else:   
                plt.text(-1, 1-4.5*dow, r'$\tau$='+str("{:.1f}".format(round(tranWC,1))), fontsize=fs, verticalalignment='top')
